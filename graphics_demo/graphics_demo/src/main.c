...
#define ROAD_TILE_W 32
#define ROAD_TILE_H 32
#define ROAD_FRAMES 3

static void drawRoadFrame(uint16_t frame)
{
	// A simple 3-frame animation by vertically offsetting the tile pattern.
	// Tile is 32px tall and repeated to fill the screen (128x160).
	uint16_t yOffset = (frame * 10) % ROAD_TILE_H; // 0, 10, 20

	for (uint16_t ty = 0; ty < 160; ty += ROAD_TILE_H)
	{
		for (uint16_t tx = 0; tx < 128; tx += ROAD_TILE_W)
		{
			// Draw the tile in two parts to wrap the y-offset.
			uint16_t partHeight = ROAD_TILE_H - yOffset;
			putImage(tx, ty, ROAD_TILE_W, partHeight, roadAnimation1 + yOffset * ROAD_TILE_W, 0, 0);
			if (yOffset != 0)
			{
				putImage(tx, ty + partHeight, ROAD_TILE_W, yOffset, roadAnimation1, 0, 0);
			}
		}
	}
}
...
int main()
{
	...
	uint16_t roadFrame = 0;
	int gameOver = 0;
	uint16_t roadTick = 0;

	while (!gameOver)
	{
		// draw moving road background (cycles through 3 frames)
		if (++roadTick >= 4)
		{
			roadTick = 0;
			roadFrame = (roadFrame + 1) % ROAD_FRAMES;
		}
		drawRoadFrame(roadFrame);
...
void loop()
{
	...
	// Update timer display (background colour ensures we overwrite previous text)
